...
public void PrintDetailedStatistics() {
    ...
    switch (survivorCount) {
        case 1:{
            ...
            {
                if (IsSurvivor(i)) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[狐狸狸Ast] \x05%N \x01[空爆: \x05%d\x01] [被扑: \x05%d\x01] [被骑: \x05%d\x01] [被撞: \x05%d\x01]\n", i, iHuntSkeets[i], timesPinned[i][ZC_HUNTER], timesPinned[i][ZC_JOCKEY], timesPinned[i][ZC_CHARGER]);
        ...
        case 3:{
            ...
            {
                if (IsSurvivor(i)) {
                    Format(tmpBuffer, sizeof(tmpBuffer), "[狐狸狸Ast] \x05%N \x01[空爆: \x05%d\x01] [被控: \x05%d\x01] [被吐: \x05%d\x01] [吃药: \x05%d\x01]\n", i, iHuntSkeets[i], totalPinned[i], boomerPops[i], pillsUsed[i]);
                    StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
                }
            }
        }
