...
public Action Menu_SITimer(int client, int args)
{
	...
	AddMenuItem(menu, "", "4s3/8s3/16s5/18s6");
	AddMenuItem(menu, "", "4s3/10s4/14s5/14s6");
	AddMenuItem(menu, "", "3s3/10s4/12s5/12s6");
	AddMenuItem(menu, "", "3s3/8s4/14s6/12s7");
	AddMenuItem(menu, "", "3s4/6s4/14s7/12s8");
	AddMenuItem(menu, "", "3/4/5/6特 特感速递");
	AddMenuItem(menu, "", "特感速递 +1特");
...
public int Menu_SITimerHandler(Handle menu, MenuAction action, int client, int param)
{
	if (action == MenuAction_Select) {
		...
		switch (param)
		{
			case 0: {
				tempSITimer = 0;
				Format(buffer, sizeof(buffer),"4s3/8s3/16s5/18s6");
				TZ_CallVoteStr(client, buffer);
			}
			case 1: {
				tempSITimer = 1;
				Format(buffer, sizeof(buffer),"4s3/10s4/14s5/14s6");
				TZ_CallVoteStr(client, buffer);
			}
			case 2: {
				tempSITimer = 2;
				Format(buffer, sizeof(buffer),"3s3/10s4/12s5/12s6");
				TZ_CallVoteStr(client, buffer);
			}
			case 3: {
				tempSITimer = 3;
				Format(buffer, sizeof(buffer),"3s3/8s4/14s6/12s7");
				TZ_CallVoteStr(client, buffer);
			}
			case 4: {
				tempSITimer = 4;
				Format(buffer, sizeof(buffer),"3s4/6s4/14s7/12s8");
				TZ_CallVoteStr(client, buffer);
			}
			case 5: {
				tempSITimer = 5;
				Format(buffer, sizeof(buffer),"3/4/5/6特感速递？");
				TZ_CallVoteStr(client, buffer);
			}
			case 6: {
				tempSITimer = 6;
				Format(buffer, sizeof(buffer),"4/5/6/7特感速递？");
				TZ_CallVoteStr(client, buffer);
			}
		}
	}
